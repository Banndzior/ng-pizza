...
export class PizzaDetailsComponent implements OnInit {
  ...
  modify(value, form) {
    ...
    this.pizzaService.modifyPizza(this.pizza).subscribe(
      () => {
  ...
  onKeyClick(event: KeyboardEvent) {
    if (event.key === "ArrowRight") {
      this.pizzaService.getPizzas().subscribe((pizzaResponse) => {
        const pizzas = pizzaResponse.value;
        const actualIndex = pizzas.findIndex(
          (pizza) => pizza.id === this.pizza.id
        );
        const nextIndex =
          actualIndex + 1 === pizzas.length ? 0 : actualIndex + 1;
        setTimeout(() => {
    ...
    if (event.key === "ArrowLeft") {
      this.pizzaService.getPizzas().subscribe((pizzaResponse) => {
        ...
        const nextIndex =
          actualIndex === 0 ? pizzas.length - 1 : actualIndex - 1;
        setTimeout(() => {
          this.getPizza(pizzas[nextIndex].id);
        }, 1000);
      });
